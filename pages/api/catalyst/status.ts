...
export default async function handler(
    ...
    try {
        ...
        const mostRecentUpdate = proposals.length > 0
            ? proposals.reduce((latest, proposal) => {
                const proposalTime = new Date(proposal.lastUpdated).getTime()
                const latestTime = new Date(latest.lastUpdated).getTime()
                return proposalTime > latestTime ? proposal : latest
            }).lastUpdated
            : null
        ...
        return res.status(200).json({
            ...
            isRecent: proposals.every(p => p.isRecent),
            timeSinceUpdate: mostRecentUpdate
                ? Math.floor((now.getTime() - new Date(mostRecentUpdate).getTime()) / 1000)
                : null
        })
    } catch (err) {
        console.error('❌ Failed to check catalyst status:', err)
        return res.status(500).json({
