...
async function fetchAndSaveContributorsAndActivity(githubToken: string) {
...
export default async (req: Request, context: Context) => {
    ...
    if (!githubToken) {
        console.log('❌ No githubToken provided, returning 400 error')
        return new Response(
            JSON.stringify({ error: 'Missing githubToken parameter' }),
            {
                status: 400,
